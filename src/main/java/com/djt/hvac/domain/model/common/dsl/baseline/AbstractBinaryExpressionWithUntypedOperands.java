package com.djt.hvac.domain.model.common.dsl.baseline;

import static java.util.Objects.requireNonNull;

abstract class AbstractBinaryExpressionWithUntypedOperands <T> extends AbstractExpression<T> {
  protected final Expression<?> expr1;
  protected final Expression<?> expr2;

  protected AbstractBinaryExpressionWithUntypedOperands(Expression<?> expr1, Expression<?> expr2) {
    this.expr1 = requireNonNull(expr1, "expr1 cannot be null");
    this.expr2 = requireNonNull(expr2, "expr2 cannot be null");
  }
  
  Expression<?> getExpr1() {
    return expr1;
  }

  Expression<?> getExpr2() {
    return expr2;
  }

  @Override
  public void accept(ExpressionVisitor visitor) {
    requireNonNull(visitor, "visitor cannot be null");
    visitor.visit(this);
    expr1.accept(visitor);
    expr2.accept(visitor);
  }
  
  
  
}
