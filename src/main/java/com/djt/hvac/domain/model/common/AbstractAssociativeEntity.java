...
import com.djt.hvac.domain.model.common.validation.NoOpResolutionStrategyImpl;
...
public abstract class AbstractAssociativeEntity extends AbstractEntity {
  ...
  public void setIsDeleted() {
    ...
    if (needsPersisting) {
      throw new IllegalStateException("Cannot set: ["
          + this
          + "] isDeleted if needsPersisting is already set.");
    }
    super.setIsDeleted();
  }
  ...
  public void setNeedsPersisting(boolean needsPersisting) {
    ...
    if (getIsDeleted()) {
      throw new IllegalStateException("Cannot set: ["
          + this
          + "] needsPersisting if isDeleted is already set.");
    }
    if (getIsModified()) {
      setNotModified();
    }
  ...
  public void validate(
    ...
    if (parentIdentities == null || parentIdentities.size() < 2) {
      ...
      if (issueTypes.contains(IssueType.INVALID_ASSOCIATIVE_ENTITY_INSTANTIATION)) {
        validationMessages.add(ValidationMessage.builder()
            ...
            .withRemediationDescription("Instantiate with at least 2 parent identities")
            .withRemediationStrategy(NoOpResolutionStrategyImpl.get())
            .build());
      }
    }
  }
