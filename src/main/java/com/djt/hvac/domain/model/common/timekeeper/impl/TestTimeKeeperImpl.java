package com.djt.hvac.domain.model.common.timekeeper.impl;
...
public class TestTimeKeeperImpl extends AbstractTimeKeeper {
  ...
  /**
   * @param date The date to set the static current time to.
   * Time doesn't advance with this implementation and can 
   * only be used in the dev-wks and dev-int environments.
   */
  ...
  public TestTimeKeeperImpl(long epochMillis) {
    ...
    if (env != null && !env.equalsIgnoreCase(LOCAL) && !env.equalsIgnoreCase(UNIT_TEST)) {
      throw new IllegalStateException("Cannot use this instance:["
          + this.getClass().getSimpleName()
          + "] in non-local/non-test environment: ["
          + env
          + "].");
    }
  ...
  public void forwardTimeInDays(int days) {
    ...
    LocalDate currentLocalDate = currentTimestamp
        .toLocalDateTime()
        .toLocalDate();
